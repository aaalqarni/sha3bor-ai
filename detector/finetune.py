...
from detector.constants import METRES, RHYMES
from detector.utils import load_tokenizer
...
def load_model(
    ...
    كذلك يتم تغيير عدد المخرجات الخاصة بالنموذج بناءً على نوع المهمة التي يجب تدريبه لحلها.
    """

    num_labels = -1

    if task_type == 'metre':
        num_labels = len(METRES)
    elif task_type == 'rhyme':
        num_labels = len(RHYMES)
...
def preprocess_examples(
    ...
    if task_type == 'metre':
        tokenized_inputs['label'] = list(map(METRES.index, examples[f'metre_label']))
    elif task_type == 'rhyme':
        tokenized_inputs['label'] = list(map(RHYMES.index, examples[f'rhyme_label']))
...
def main(args: argparse.Namespace) -> None:
    ...
    # تجهيز النموذج.
    model = load_model(args.model_name, args.task_type, tokenizer)

    # تجهيز البيانات.
    tokenized_dataset = load_and_prepare_dataset(args.input_file, args.task_type, tokenizer)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=['metre', 'rhyme'],
        required=True,
        help='نوع المهمة التي يجب تدريب النموذج لحلها',
    )
    parser.add_argument(
        '--input_file',
    ...
    parser.add_argument('--output_path', default='detector/models/results', help='مسار حفظ النموذج بعد التدريب')
